...
# =====================================================================
HEADER_TEXT = "JURNAL TEKNIK ITS Vol. 15, No. 1, (2026) ISSN: 2337-3539 (2301-9271 Print)"
...
# Heading 2: spasi sebelum/sesudah 6/3pt (sesuai TEMPLATE_PUBLIKASI_TEKNIK).
st_h2 = style_by_name("Heading 2")
if st_h2 is not None:
    st_h2.paragraph_format.space_before = Pt(6)
    st_h2.paragraph_format.space_after = Pt(3)
    print("Heading 2 -> space-before 6pt, space-after 3pt.")
...
# --- 0c) spacing sekitar gambar/tabel: cegah tabrakan dengan teks/keterangan ---
# Spacing 6pt = ~4,2 mm. Template: caption gambar rata justify di bawah;
# caption tabel rata tengah di atas. Gambar dalam paragraf sendiri (inline).
for p in doc.paragraphs:
    txt = p.text.strip()
    has_drawing = p._p.find('.//' + qn('w:drawing')) is not None
    if has_drawing:
        p.paragraph_format.space_before = Pt(3)    # sedikit ruang dari teks atas
        p.paragraph_format.space_after = Pt(0)     # rapat ke caption di bawah
    elif txt.startswith("Gambar "):
        p.paragraph_format.space_before = Pt(0)    # rapat ke gambar di atas
        p.paragraph_format.space_after = Pt(6)     # ruang sebelum konten berikutnya
    elif txt.startswith("Tabel "):
        p.paragraph_format.space_before = Pt(8)    # ruang dari konten sebelumnya
        p.paragraph_format.space_after = Pt(0)     # rapat ke tabel di bawah

# paragraf pertama setelah tabel: beri ruang agar tidak menempel ke border bawah
body = doc.element.body
prev_was_tbl = False
for child in body:
    ...
    if tag == 'tbl':
        prev_was_tbl = True
    elif tag == 'p' and prev_was_tbl:
        pPr = child.find(qn('w:pPr'))
        if pPr is None:
            pPr = OxmlElement('w:pPr'); child.insert(0, pPr)
        spacing = pPr.find(qn('w:spacing'))
        if spacing is None:
            spacing = OxmlElement('w:spacing'); pPr.append(spacing)
        # Cek apakah paragraf setelah tabel adalah caption Tabel lain (tabel berurutan)
        txt = child.text.strip() if child.text else ''
        for r in child.iter(qn('w:t')):
            txt += r.text if r.text else ''
        txt = txt.strip()
        if txt.startswith("Tabel "):
            spacing.set(qn('w:before'), '60')    # ~3pt (tabel berurutan, rapatkan)
        else:
            spacing.set(qn('w:before'), '120')   # ~6pt (teks setelah tabel, normal)
        prev_was_tbl = False
    else:
        prev_was_tbl = False

print("Spacing gambar/tabel dipasang: cegah tabrakan caption & konten.")
...
# Tabel panjang (>20 baris) tidak dicegah terpotong agar bisa mengalir
# antar halaman (longtable). Tabel kecil tetap dijaga utuh.
LONG_TABLE_ROW = 8
for tbl in doc.tables:
    rows = tbl.rows
    n = len(rows)
...
for p in doc.paragraphs:
    if p.text.strip().startswith("Tabel "):
        ...
        p.paragraph_format.alignment = AL.CENTER       # penyelarasan tengah (template)
...
for tbl in doc.tables:
    ...
    tblInd.set(qn('w:type'), 'dxa'); tblInd.set(qn('w:w'), '108')
    # hapus jc (bertentangan dengan tblInd)
    ...
    # --- border horizontal gaya buku TA (\toprule / \midrule / \bottomrule) ---
    # Tambahkan garis horizontal: atas, bawah, dan bawah baris header.
    # Untuk tabel dengan header 2 baris (row[1] sel pertamanya kosong),
    # garis bawah header ditaruh di row[1] (sub-header), bukan row[0].
    # Tanpa garis vertikal (kiri/kanan/dalam).
    BORDER_SZ = '4'   # 0,5pt (~4 eighths of a point)
    ...
    for edge in ('top', 'bottom'):
        ...
        borders.append(el)
    # garis bawah header row (deteksi header 2 baris)
    if tbl.rows:
        hdr = tbl.rows[0]
        if len(tbl.rows) > 1:
            # cek apakah row[1] sel pertama kosong -> header 2 baris
            r1_text = ''.join(p.text for p in tbl.rows[1].cells[0].paragraphs).strip()
            if not r1_text:
                hdr = tbl.rows[1]   # garis bawah sub-header
...
# --- 8) LAMPIRAN / UCAPAN TERIMA KASIH / DAFTAR PUSTAKA -> centered, no bold ---
REF_HEAD_LABELS = ("LAMPIRAN", "UCAPAN TERIMA KASIH", "DAFTAR PUSTAKA")
n_ref = 0
for p in doc.paragraphs:
    if p.style.name != "Heading 1":
        continue
    txt = p.text.strip()
    if txt in REF_HEAD_LABELS:
        p.style = style_by_name("Normal")
        p.alignment = AL.CENTER                    # centered
        p.paragraph_format.space_before = Pt(18)
        p.paragraph_format.space_after = Pt(6)
        p.paragraph_format.first_line_indent = Pt(0)
        for r in p.runs:
            r.font.bold = False                   # no bold
            r.font.italic = False
            r.font.size = Pt(10)
            r.font.name = "Times New Roman"
        n_ref += 1
print("Reference Head (LAMPIRAN/UCAPAN/DAFTAR PUSTAKA) -> centered, no bold:", n_ref)
